...
public class WelcomePageIndicator extends RecyclerView.ItemDecoration {

    private final Paint activePaint = new Paint();
    private final Paint inactivePaint = new Paint();
    private final float indicatorRadius;

    public WelcomePageIndicator(@NonNull Context context) {
        final DisplayMetrics dm = context.getResources().getDisplayMetrics();
        indicatorRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, dm);

        inactivePaint.setStyle(Paint.Style.FILL);
        inactivePaint.setColor(0x44fffff);
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if (adapter != null) {
            ...
            if (lm instanceof LinearLayoutManager) {
                currentIndicator = ((LinearLayoutManager) lm)
                        .findFirstCompletelyVisibleItemPosition();
            }
            for (int i = 0; i < adapter.getItemCount(); i++) {
                drawIndicator(c, posX + 4 * indicatorRadius * i, posY,
                        currentIndicator == i);
            }
        }
    }

    void drawIndicator(@NonNull Canvas c, float x, float y, boolean active) {
