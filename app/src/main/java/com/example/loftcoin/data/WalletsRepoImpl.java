...
public class WalletsRepoImpl implements WalletsRepo {
    ...
    @NonNull
    @Override
    public Completable addWallet(@NonNull Currency currency, List<Integer> ids) {
        return coinsRepo.nextCoin(currency, ids)
                .map((coin) -> {
                    Map<String, Object> data = new HashMap<>();
                    data.put("balance", 100 * (random.nextDouble() + 0.1));
